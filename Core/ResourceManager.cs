...
namespace Imprint.Core
{
    ...
    public abstract class BaseResource : ICloneable, INotifyPropertyChanged
    {
        ...
        {
            if (hd == null) return;
            foreach (PropertyChangedEventHandler hdl in hd)
            {
                hdl.Invoke(this, new PropertyChangedEventArgs(PName));
            }
        }

        private List<PropertyChangedEventHandler> hd = new List<PropertyChangedEventHandler>();
